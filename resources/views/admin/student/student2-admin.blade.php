<x-admin-dashboard>
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 p-4">
            ...
            <div class="w-full md:w-1/2 flex items-center">
                <form action="{{ url('/admin/students/student-admin') }}" method="GET"
                ...
                <!-- Tombol Back -->
                @if (request()->has('q'))
                    <a href="{{ url('/admin/students/student-admin') }}"
                        class="ml-2 text-white bg-gray-500 hover:bg-gray-600 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-gray-600 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800">
                        X
                    </a>
                @endif
            </div>
        ...
        <nav class="flex flex-col md:flex-row justify-between items-start md:items-center space-y-3 md:space-y-0 p-4"
            ...
            <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                Showing
                <span
                    class="font-semibold text-gray-900 dark:text-white">{{ $students->firstItem() }}-{{ $students->lastItem() }}</span>
                of
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function(event) {
            ...
            const modalEditButton = modal.querySelector('a[type="button"]');
            ...
            modalDetailBtns.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    const id = button.getAttribute('data-id'); // Ambil ID siswa
